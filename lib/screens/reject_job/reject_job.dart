...
class RejectJob extends StatefulWidget {
  const RejectJob({Key? key}) : super(key: key);
...
class _RejectJobState extends State<RejectJob> {
  ...
  getListReady() {
    ...
    if (jobController.templates[AppTexts.tempRejectJob] != null &&
        jobController.templates[AppTexts.tempRejectJob].isNotEmpty) {
      for (int i = 0;
          i < jobController.templates[AppTexts.tempRejectJob].length;
          i++) {
        if (jobController.templates[AppTexts.tempRejectJob][i]['status']
                .toString() ==
            "1") {
          itemsID.add(
              jobController.templates[AppTexts.tempRejectJob][i]['id'] != null
                  ? jobController.templates[AppTexts.tempRejectJob][i]['id']
                      .toString()
                  : "");
          items.add(jobController.templates[AppTexts.tempRejectJob][i]
                  ['title'] ??
              "");
          itemsDescription.add(jobController.templates[AppTexts.tempRejectJob]
                  [i]['description'] ??
              " ");
        }
      }
          ...
          onPressed: () {
            if (formKey.currentState!.validate()) {
              ...
              customDialog(
                ...
                middleText: AppTexts.rejectJobConfirmation,
